...
개발자는 애플리케이션을 더 큰 부분의 애플리케이션을 구축하는데 사용할 수 있는 모듈, 구성 요소 및 기능으로 나눕니다. 이러한 내부 모듈과 외부 서드파티 패키지를 내보내고 가져오면서 파일 종속성의 복잡한 웹이 생성됩니다.

Bundling은 사용자가 웹 페이지를 방문할 때 파일에 대한 요청 수를 줄이는 것을 목표로 웹 종속성을 해결하고 파일(또는 모듈)을 브라우저에 최적화된 번들로 병합(또는 패키징)하는 프로세스입니다.
